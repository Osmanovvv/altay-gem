...
export const CONTENT_TYPE_HINTS: Record<string, ModelHints> = {
  'api::product.product': {
    ...
    oldPriceRub: {
      label: 'Старая цена, ₽',
      description:
        'Для перечёркнутой цены. Если больше текущей — на карточке появится процент скидки.',
    },
    portionMassG: {
    ...
    deliveryWeightG: {
      ...
      description: 'Вес одной единицы — по нему считается стоимость доставки.',
    },
    heroProduct: {
  ...
  'api::promo.promo': {
    ...
    conditions: {
      label: 'Условия',
      description: 'Список условий: «от 3000 ₽», «до 31 декабря».',
    },
    promocode: {
      label: 'Промокод',
