...
/**
 * Подпись кнопки отправки заказа. Онлайн-оплата ведёт на страницу платежа,
 * остальные способы просто оформляют заказ — и покупатель должен видеть,
 * что именно произойдёт по нажатию.
 *
 * Неизвестный способ трактуем как оплату на месте: «Оформить заказ» — честное
 * описание для любого не-онлайн сценария, а падать на опечатке форма не должна.
 */
export function submitLabelFor(payment: string): string {
  return payment === "online" ? "Оплатить" : "Оформить заказ";
}

/** Текст ссылки на политику — один на оба канала. */
export const CONSENT_LINK_TEXT = "политикой обработки персональных данных";

/**
 * Согласие на обработку ПД (152-ФЗ) даётся конкретным действием, поэтому текст
 * называет РОВНО ту кнопку, которая нарисована рядом, — отсюда общий источник
 * подписи с `submitLabelFor`. Ссылка на политику дорисовывается компонентом:
 * здесь только текст, без React.
 */
export function consentPrefix(payment: string): string {
  return `Нажимая «${submitLabelFor(payment)}», вы соглашаетесь с`;
}
