...
export const evotorProducts = pgTable(
  'evotor_products',
  {
    ...
    raw: jsonb('raw'),
    /**
     * Время последнего УЧТЁННОГО в остатке движения или снимка: документа чека
     * (дельта либо абсолют «чека-как-сверки»), либо файла выгрузки (его mtime).
     * Охраняет порядок для АБСОЛЮТОВ: снимок старее метки не применяется —
     * иначе стёр бы более свежие движения. Дельты применяются безусловно
     * (коммутативны) и лишь двигают метку вперёд. null — движений не видели.
     */
