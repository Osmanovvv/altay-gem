...
/**
 * Штрихкоды приходят из ДВУХ источников с разной формой: из xlsx — строкой
 * («4607068350021 2000000000060»), из Облака Эвотора — уже массивом.
 *
 * Живой инцидент 09.08.2026: облачный источник отдавал массив, парсер понимал
 * только строку и молча возвращал пустой список. Штрихкод потерялся у всех
 * 3600 товаров, ключ сопоставления откатился на артикул — а артикулы Эвотора
 * у РАЗНЫХ товаров совпадают, и под одним ключом склеились чужие карточки
 * («Восковая моль», «Краковская Гродно», «Сервелат Царский»). Витрина
 * суммировала остатки посторонних товаров.
 */
describe('parseGoodsExportRow: форма штрихкодов', () => {
  const base = {
    uuid: 'c5f72831-aa24-4e0e-ab81-0c27401c9280',
    Наименование: 'Мёд горный',
    Артикул: 'ART-1',
  };

  it('строка с разделителями (xlsx) — как раньше', () => {
    const p = parseGoodsExportRow({ ...base, 'Штрих-код': '4607068350021 2000000000060' });
    expect(p!.barcodes).toEqual(['4607068350021', '2000000000060']);
    expect(p!.matchKey).toBe('4607068350021'); // ключ — первый штрихкод, не артикул
  });

  it('МАССИВ (облако Эвотора) — тоже принимается', () => {
    const p = parseGoodsExportRow({ ...base, 'Штрих-код': ['4607068350021', '2000000000060'] });
    expect(p!.barcodes).toEqual(['4607068350021', '2000000000060']);
    expect(p!.matchKey).toBe('4607068350021');
  });

  it('массив с мусором: пустые и нестроковые элементы отбрасываются', () => {
    const p = parseGoodsExportRow({ ...base, 'Штрих-код': ['', '  ', null, 4607068350021] });
    expect(p!.barcodes).toEqual(['4607068350021']); // число приводится к строке
  });

  it('пустой массив — как отсутствие: откат ключа на артикул', () => {
    const p = parseGoodsExportRow({ ...base, 'Штрих-код': [] });
    expect(p!.barcodes).toEqual([]);
    expect(p!.matchKey).toBe('ART-1');
  });
});
