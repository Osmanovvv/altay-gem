...
/**
 * Штрихкоды приходят в двух формах: из xlsx — строкой с разделителями,
 * из Облака Эвотора — уже массивом. Принимаем обе.
 *
 * Массив ОБЯЗАТЕЛЬНО обрабатывать явно: str() на массиве возвращает null, и
 * штрихкоды молча терялись бы целиком. Так и случилось 09.08.2026 — ключ
 * сопоставления откатился на артикул, а артикулы Эвотора у разных товаров
 * совпадают, и под одним ключом склеились чужие карточки. См. spec.
 */
const barcodesOf = (v: unknown): string[] => {
  const parts = Array.isArray(v)
    ? v.map(str).filter((s): s is string => s !== null)
    : (str(v) ?? '').split(/[,;\s]+/);
  return parts.map((b) => b.trim()).filter((b) => b !== '');
};
