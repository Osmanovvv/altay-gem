 ...
 *
 * ЮKassa и принимает деньги (эквайер), и бьёт фискальный чек 54-ФЗ («Чеки от
 * ЮKassa»): чек уходит объектом receipt в запросе платежа ЛИБО отдельным
 * POST /receipts после сборки (маркировка). Отдельной интеграции с «Цифровой
 * кассой» Эвотора нет — это согласованное отклонение от ТЗ (см. «ПРИЁМКА —
 * Этап 3 — отклонения.md»). Без ключей — no-op (заказ создаётся без paymentUrl),
