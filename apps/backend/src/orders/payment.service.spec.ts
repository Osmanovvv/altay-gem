...
describe('фиксы ревью шага 4: идемпотентность чека и статусы', () => {
  ...
  it('findReceiptByPayment: чек ПРЕДОПЛАТЫ НЕ считается фискализацией (дизайн B)', async () => {
    // Маркированный заказ: при оплате уходит чек предоплаты (full_prepayment).
    // Он НЕ должен маскировать отсутствие чека зачёта — иначе fiscalizeOrder
    // решит «уже фискализировано» и коды маркировки в ЧЗ не уйдут.
    globalThis.fetch = okFetch({
      items: [
        {
          id: 'rc_prepay',
    ...
    // Есть и предоплата, и чек ЗАЧЁТА (full_payment) — возвращаем именно зачёт.
    globalThis.fetch = okFetch({
      items: [
        {
          id: 'rc_prepay',
        ...
        },
        {
          ...
          type: 'payment',
          status: 'succeeded',
          items: [{ payment_mode: 'full_payment' }],
        },
      ],
    });
    expect(await s.findReceiptByPayment('pay_9')).toEqual({
