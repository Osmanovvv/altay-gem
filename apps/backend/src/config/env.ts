...
const envSchema = z.object({
  ...
  // Страховка от УСТАРЕВШЕЙ выгрузки: выгрузка, СНЯТАЯ раньше N часов, сверкой
  // не применяется (иначе откатит остатки, насчитанные чеками, к старому
  // снимку) — пропуск и алерт. Возраст считается от момента снятия из
  // содержимого файла, поэтому в него входит и дорога от товароучётки до нас
  // (живой случай — 6,5 ч). По умолчанию 48 ч = сутки + доставка + запас.
  // 0 — проверку выключить.
