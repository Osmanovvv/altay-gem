...
await check('POST /delivery/calculate — сводка ДО оплаты совпадает с тарифами', async () => {
  // курьер, ниже порога: 2 × 1260 = 2520 < 3000 -> 300₽
  const a = (await api('POST', '/delivery/calculate', {
    deliveryMethod: 'courier_nsk',
    items: [{ id: 'pantogematogen-250-ml', quantity: 2 }],
  }, {}, 200)).body;
  assert(a.deliveryRub === 300 && a.totalRub === 2820, JSON.stringify(a));
  // курьер, выше порога с учётом скидки: 3 × 1260 = 3780, -10% = 3402 >= 3000 -> 0₽
  const b = (await api('POST', '/delivery/calculate', {
    deliveryMethod: 'courier_nsk',
    items: [{ id: 'pantogematogen-250-ml', quantity: 3 }],
    promoCode: 'ALTAI10',
  }, {}, 200)).body;
  assert(b.deliveryRub === 0 && b.discountRub === 378, JSON.stringify(b));
  // Россия по весу: мука 2000 г ×1 -> тир «до 3000 г» = 650₽
  const c = (await api('POST', '/delivery/calculate', {
    deliveryMethod: 'russia',
    items: [{ id: 'muka-monastyrskiy-dvor-2-kg', quantity: 1 }],
  }, {}, 200)).body;
  assert(c.weightG === 2000 && c.deliveryRub === 650, JSON.stringify(c));
  // самовывоз всегда 0
  const d = (await api('POST', '/delivery/calculate', {
    deliveryMethod: 'pickup_titova',
    items: [{ id: 'muka-monastyrskiy-dvor-2-kg', quantity: 1 }],
  }, {}, 200)).body;
  assert(d.deliveryRub === 0, 'самовывоз');
  // скоропорт по России — 400
  await api('POST', '/delivery/calculate', {
    deliveryMethod: 'russia',
